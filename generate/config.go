...
type Config struct {
	...
	//
	// [genqlient.yaml docs]: https://github.com/HwangTaehyun/genqlient/blob/main/docs/genqlient.yaml
	...
	if err != nil {
		...
		if c.Package != "" {
			...
			return errorf(nil, "unable to guess package-name: %v"+
				...
				"\nExample: https://github.com/HwangTaehyun/genqlient/blob/main/example/genqlient.yaml#L6", err)
		}
	} else { // err == nil
