...
func (g *generator) convertDefinition(
	...
	case ast.InputObject:
		...
		for i, field := range def.Fields {
			...
			// Try both the original name and lowercase version since GraphQL schema uses lowercase
			typesToCheck := []string{def.Name}
			...
			for _, typeName := range typesToCheck {
				if g.globalFieldDirectives[typeName] != nil {
					if globalFieldDirective := g.globalFieldDirectives[typeName][field.Name]; globalFieldDirective != nil {
						...
						break // Found directive, no need to check other type names
					}
				}
			}
			...
			if !g.Config.StructReferences {
				...
				// pass these validations. See https://github.com/HwangTaehyun/genqlient/issues/342
	...
	case ast.Scalar:
		...
		return nil, errorf(
			pos, "unknown scalar %v: please add it to \"bindings\" in genqlient.yaml"+
				"\nExample: https://github.com/HwangTaehyun/genqlient/blob/main/example/genqlient.yaml#L12", def.Name)
	default:
			...
			case *goStructType, *goInterfaceType:
				...
				return nil, errorf(nil,
					"genqlient doesn't allow duplicate fields with different selections "+
						"(see https://github.com/HwangTaehyun/genqlient/issues/64); "+
